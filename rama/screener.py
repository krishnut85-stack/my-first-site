...
class Industry:
    ...
    catalyst_score: float = 0.0      # -1..+1 insider/bulk-deal tilt (catalysts.py)
...
def top_industries(n=None, max_pe=None, csv_path=None) -> list[Industry]:
    ...
    # Tilt the ranking by the insider/catalyst signal BEFORE the top-N slice, so
    # a group insiders are accumulating can climb into the picks (and one they
    # are dumping can drop out). No-op when there is no catalyst data.
    if config.USE_CATALYST_SIGNAL:
        from .catalysts import apply_catalyst_tilt
        ranked = apply_catalyst_tilt(ranked)
    picks = []
    for i in ranked:
