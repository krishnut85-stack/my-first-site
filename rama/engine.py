...
def run_paper_session(verbose: bool = True, csv_path=None) -> dict:
    ...
    ranked_symbols = []
    symbol_meta = {}                      # symbol -> its Industry (for the audit gate)
    for pick in picks:
    ...
    # SIZE: flat cap or fractional-Kelly from Rama's own measured edge.
    from .sizing import position_budget, target_fraction
    ...
    au_uptrend = uptrend or not config.USE_REGIME_FILTER  # regime already handled
    ...
    if not regime_blocked:
        for sym in ranked_symbols:
            ...
            if ltp is None:
                continue
            verdict = audit_candidate(sym, symbol_meta.get(sym), ltp,
                                      uptrend=au_uptrend,
                                      catalyst_score=cat_scores.get(sym))
            if not verdict.ok:
                vetoes.append((sym, verdict.reason))
                continue
...
def _print(r: dict) -> None:
    pf = r["portfolio"]
    print("\n" + "=" * 66)
    print(f"  {config.BOT_NAME} · PAPER PORTFOLIO" + ("  (REAL Kite prices)" if r["real_data"]
    ...
    if r.get("entries"):
        print(f"  Sizing           : {r.get('sizing_label', 'flat cap')}")
        print("  Entries this run:")
        for s, ltp, qty in r["entries"]:
            print(f"    BUY  {s:12} @ {ltp:>9.2f}  x{qty}")
    if r.get("vetoes"):
