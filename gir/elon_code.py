...
_TOKENS = {}        # sym -> instrument_token, captured from quote() (saves an ltp call)
...
def fetch_quotes(kite, symbols):
    ...
    for i in range(0, len(symbols), CHUNK):
        ...
        for s in batch:
            ...
            if tok:
                _TOKENS[s] = tok   # reuse for the trend check -> no extra ltp() call
            out[s] = {
...
def _in_uptrend(kite, sym):
    ...
    try:
        ...
        # Reuse the instrument_token captured in fetch_quotes; only fall back to
        # an ltp() call if we somehow don't have it (saves one API hit per turn).
        tok = _TOKENS.get(sym)
        if not tok:
            tok = kite.ltp([f"NSE:{sym}"]).get(f"NSE:{sym}", {}).get("instrument_token")
        if tok:
            to = _dt.datetime.now()
            frm = to - _dt.timedelta(days=TREND_HIST_BARS * 2 + 10)
            bars = kite.historical_data(tok, frm, to, "day")
            # CRITICAL: drop today's still-forming daily candle. Intraday, Kite's
            # last "day" bar is today's LIVE price -- and we only reach here for a
            # stock that is DOWN and dipping, so today's depressed price would make
            # a genuine uptrend look like a downtrend and wrongly reject the setup.
            # The trend must be judged on the last COMPLETED close vs an SMA of
            # completed closes.
            completed = [b for b in bars
                         if str(b.get("date"))[:10] != today]
