...
def test_state_exposes_options_book(tmp_path, monkeypatch):
    ...
    # the two standalone books (options + the Swaminatha news book) are always
    # exposed and folded into the grand-total capital alongside the equity books
    sw = st["swaminatha"]
    assert sw["key"] == "swaminatha" and sw["strategy"] == "swaminatha"
    assert sw["capital"] == config.SWAMINATHA_CAPITAL and sw["n"] == 0
    assert st["totals"]["capital"] == (len(PROFILES) + 2) * 1_000_000  # equity books + options + news
